...
const POSModule = ({ onBack }) => {
    ...
    const handleScanQR = async (code) => {
        ...
        const rif = currentUser?.rif || RIF_DEFAULT;
        try {
            // Buscar en tabla products por QR, barcode o codigo
            const results = await sbFetch(`products?rif_empresa=eq.${encodeURIComponent(rif)}&or=(codigo_qr.eq.${encodeURIComponent(code)},codigo_barra.eq.${encodeURIComponent(code)},codigo_producto.eq.${encodeURIComponent(code)})&activo=eq.true`);
            if (results && results.length > 0) {
                const p = results[0];
                // Buscar en inventario contable para tener cost
                const invProd = inventory.find(i => i.name === p.nombre);
                const prodToAdd = {
                    id: `db-${p.id}`, name: p.nombre,
                ...
                // Fallback: buscar por nombre en inventario contable
                const inv = inventory.find(i => i.name === code || i.id === code);
                if (inv) { addToCart(inv, 1); setScanFeedback(`✓ ${inv.name} agregado`); }
                else setScanFeedback(`⚠ No encontrado: ${code.substring(0,20)}`);
            }
        } catch(e) {
            setScanFeedback('⚠ Error de búsqueda');
            console.error(e);
        }
        ...
        <div className="relative">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in h-[90vh]">
                <div className="lg:col-span-7 flex flex-col space-y-4 h-full">
                    <div className="bg-white p-4 rounded-[2rem] shadow-sm border border-slate-100 flex gap-3 items-center">
                        ...
                        <button onClick={()=>setScannerOpen(true)} className="flex items-center gap-2 bg-slate-900 hover:bg-black text-white px-4 py-3 rounded-2xl text-[10px] font-black uppercase transition-colors flex-shrink-0">
                            <Icon name="ScanLine" size={16}/> <span className="hidden sm:inline">Escanear QR</span>
                        </button>
                        <div className="text-right px-3 border-l border-slate-100 flex-shrink-0">
                            <p className="text-[10px] font-bold text-slate-400 uppercase">Tasa BCV</p>
                            <p className="font-black text-blue-600 text-sm">{tasaBCV} Bs/$</p>
                        </div>
                    </div>
                    {scanFeedback && (
                        <div className="bg-slate-900 text-white px-4 py-2.5 rounded-xl text-xs font-black uppercase animate-in text-center">
                            {scanFeedback}
                        </div>
                    )}
                    {scannerOpen && <QRScanner onDetected={handleScanQR} onClose={()=>setScannerOpen(false)}/>}
